...
import { 
  ...
  X, RefreshCcw, Info, Download, Edit3, LayoutDashboard, Save
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    if (!user || !db) return;
    const marketRef = doc(db, 'artifacts', appId, 'public', 'data', 'market_data', 'global');
    const geoRef = doc(db, 'artifacts', appId, 'public', 'data', 'geoStore', 'registry');
    ...
    const unsubGeo = onSnapshot(geoRef, (snap) => {
      if (snap.exists()) setRegistry(snap.data());
    });
    return () => { unsubMarket(); unsubGeo(); };
  }, [user]);

  // NEW: Function to sync manual edits back to the cloud
  const syncEditsToCloud = async () => {
    if (!user || !db) return;
    setIsSyncing(true);
    try {
  ...
  // NEW: Handle individual cell changes in the editor
  const handleCellChange = (index, field, value) => {
    const updatedData = [...data];
    updatedData[index][field] = value;
    
    // Recalculate derived values if necessary (capacity, tags)
    if (field === 'Estimated Capacity') {
        let capVal = parseFloat(value.replace(/,/g, '')) || 0;
        if (value.toLowerCase().includes('u/d')) capVal *= 350;
        if (value.toLowerCase().includes('mil')) capVal *= 1000000;
        updatedData[index].capacityValue = capVal;
    }
    
    setData(updatedData);
  };

  const exportMasterDataset = () => {
    ...
    const csvRows = data.map(row => {
      ...
      const coords = registry[query] || ["", ""];
      return headers.map(header => {
        let val = (header === 'lat') ? (row.lat || coords[0]) : (header === 'lng') ? (row.lng || coords[1]) : (row[header] ?? "");
    ...
    link.href = url;
    link.download = `Master_Tire_Intel_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const processRawCSV = useCallback(async (csvText, name = "MarketData.csv") => {
    ...
    if (user && db) {
      try {
        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'market_data', 'global');
        await setDoc(docRef, { rows, fileName: name, updatedAt: new Date().toISOString(), uploadedBy: user.uid });
  ...
  const stats = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      capacities[c] = (capacities[c] || 0) + (d.capacityValue || 0);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-10">
      {hoveredTag && (
        <div className="fixed z-[9999] bg-slate-900 text-white text-[10px] font-bold px-3 py-2 rounded shadow-xl pointer-events-none transform -translate-x-1/2 -translate-y-full mb-2" style={{ left: hoveredTag.x, top: hoveredTag.y }}>
          <div className="flex items-center gap-2"><span className="opacity-50">{hoveredTag.code}:</span><span>{hoveredTag.label}</span></div>
      ...
      <nav className="bg-white border-b border-slate-200 px-4 py-2 sticky top-0 z-[100] flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">Status: <span className={user ? "text-emerald-500" : "text-amber-500"}>{user ? "SYNCED" : "CONNECTING"}</span></p>
            </div>
          </div>

          {/* TAB SWITCHER */}
          <div className="flex bg-slate-100 p-1 rounded-lg">
             <button onClick={() => setActiveTab('dashboard')} className={`flex items-center gap-2 px-3 py-1.5 rounded text-[9px] font-black uppercase transition-all ${activeTab === 'dashboard' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
                <LayoutDashboard size={12} /> Dashboard
             </button>
             <button onClick={() => setActiveTab('editor')} className={`flex items-center gap-2 px-3 py-1.5 rounded text-[9px] font-black uppercase transition-all ${activeTab === 'editor' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}>
                <Edit3 size={12} /> Data Manager
             </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {activeTab === 'editor' && (
             <button onClick={syncEditsToCloud} disabled={isSyncing} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white rounded text-[9px] font-black uppercase hover:bg-blue-700 transition-all shadow-md">
                {isSyncing ? <RefreshCcw size={12} className="animate-spin" /> : <Save size={12} />}
                {isSyncing ? 'Syncing...' : 'Sync to Cloud'}
             </button>
          )}
          {data.length > 0 && (
            <button onClick={exportMasterDataset} className="flex items-center gap-2 px-3 py-1.5 bg-slate-900 text-white rounded text-[9px] font-black uppercase hover:bg-slate-800 transition-all shadow-sm">
          ...
          <div className="relative border rounded p-1 border-slate-200 bg-slate-50 w-40">
            ...
            <div className="flex items-center gap-2 px-1">
              <Upload size={10} className="text-slate-400 shrink-0" /><span className="text-[9px] font-bold text-slate-600 truncate">{fileName || "Upload CSV"}</span>
            </div>
          </div>
          {stats?.isFiltered && (
            <button onClick={() => setActiveFilters({ Region: [], Company: [], Country: [], TireTypes: [] })} className="px-2 py-1.5 bg-red-600 text-white rounded text-[9px] font-bold hover:bg-red-700 flex items-center gap-1 shadow-sm"><RefreshCcw size={10} /> RESET</button>
          )}
        </div>
      </nav>
      ...
      <main className="max-w-full mx-auto px-4 py-4">
        {!data.length ? (
          <div className="flex flex-col items-center justify-center py-32 bg-white rounded-xl border border-dashed border-slate-200">
            ...
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">{user ? "Awaiting Data Feed..." : "Authenticating..."}</h2>
          </div>
        ) : activeTab === 'dashboard' ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              ...
              <KpiCard icon={<Layers size={14}/>} title="Capacity" value={(filteredData.reduce((s, d) => s + (d.capacityValue || 0), 0) / 1000000).toFixed(1) + 'M'} />
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between bg-white z-[60]">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search registry..." className="pl-7 pr-3 py-1 bg-slate-50 border border-slate-200 rounded text-[10px] w-48 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
              </div>
          ...
          /* --- EDITOR VIEW (New Integrated Logic) --- */
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
               <div>
                  <h2 className="text-xs font-black uppercase tracking-widest text-slate-800">Master Data Manager</h2>
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Edit records below and sync directly to cloud storage.</p>
               </div>
               <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                  <input type="text" placeholder="Filter rows to edit..." className="pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-xs w-64 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
               </div>
            </div>
               ...
               <table className="w-full text-left border-collapse min-w-[1200px]">
                  <thead className="sticky top-0 bg-white z-20 shadow-sm">
                     <tr className="border-b border-slate-200">
                        {['Region', 'Company', 'Country', 'Plant Location', 'Year Opened', 'Estimated Capacity', 'Tire Types'].map(h => (
                           <th key={h} className="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-tighter">{h}</th>
                        ))}
                     </tr>
                  </thead>
                     ...
                     {filteredData.map((row, idx) => {
                        ...
                        return (
                           <tr key={idx} className="hover:bg-slate-50/50">
                              {['Region', 'Company', 'Country', 'Plant Location', 'Year Opened', 'Estimated Capacity', 'Tire Types'].map(field => (
                                 <td key={field} className="px-2 py-1">
                                    <input 
                                       className="w-full px-2 py-2 text-[11px] font-medium text-slate-700 bg-transparent hover:bg-white focus:bg-white border border-transparent hover:border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 rounded outline-none transition-all"
                                       value={row[field] || ''}
                                       onChange={(e) => handleCellChange(originalIndex, field, e.target.value)}
                                    />
                                 </td>
                              ))}
                           </tr>
...
// --- Sub-components (Restored from your original) ---
    ...
    <th className="px-3 py-3 relative group" style={{ width }}>
      <div className="flex items-center justify-between gap-1">
        <div className="flex items-center gap-1 cursor-pointer overflow-hidden" onClick={() => onSort(field)}>
          ...
          {sort.key === field && (sort.direction === 'asc' ? <ChevronUp size={10} className="text-blue-600 shrink-0" /> : <ChevronDown size={10} className="text-blue-600 shrink-0" />)}
        </div>
